...
%             end
            ...
            m = 0;
            r = 0.2;
            angle_a = atan2(dv(2), dv(1));
            
            sq_s = get_square_spiral(k, m, r, angle_a);
            sq_s = center + sq_s;
            ...
            % the square spiral to the next point of the ellipse spiral (a
            % point which is ahead!)
            ss = 1.5;
            
            while norm(double(subs(spiral, gamma, ss)) - x(:,end), 2) >= 0.5
                ss = ss + 0.1;
            end
